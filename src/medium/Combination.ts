...
type tuple2Union<S extends string[]> = S extends [infer F, ...infer R]
	? F | tuple2Union<R extends string[] ? R : []>
	: never;
type AllCombination<
	S extends string[],
	U extends string = tuple2Union<S> & string,
	T extends string = U
> = [U] extends [never]
	? ''
	: U extends T
	? '' | `${U} ${AllCombination<never, Exclude<T, U>>}`
	: never;
import { TrimRight } from './TrimRight';
type Combination<T extends string[]> = Exclude<
	TrimRight<AllCombination<T>>,
	''
>;
